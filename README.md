...
Xwayland; `SOUNDSCAPE_NO_FRAME_GATE=1` disables the hidden-window frame gate (see docs/design.md); `SOUNDSCAPE_PROFILE=1` prints per-phase frame timings and pulse/band-level stats at exit (`SOUNDSCAPE_PUNCH_LEVEL=lo,hi` / `SOUNDSCAPE_PUNCH_BINS=lo,hi` override the center-pulse gate and band for experiments; `build/punch-calibrate song.f32 …` sweeps them offline).
